...
class DataTrainingArguments:
    ...
    train_file: Optional[str] = field(metadata={"help": "The input training data file (a text file)."})
...
def print_avg_scores(scores):
    ...
    # Now print all the scores
    print(f"Runs: {num_runs}")
    print(f"All scores: {string_scores}")
    print(f"Highest score: {highest}")
    print(f"Lowest score : {lowest}")
    print(f"Average score: {avg}")
    print(f"Average rounded score: {round(avg,1)}")
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        print ("Reading all arguments from JSON input file")
        model_args, data_args, training_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        raise ValueError("Script only tested with single JSON file as argument")
    ...
    # Do multiple runs inside the training script to make things easier
    NUM_RUNS = 10
    seeds = [111, 222, 333, 444, 555, 666, 777, 888, 999, 11, 22, 33, 44, 55, 66, 77, 88]
    ...
    for idx in range(NUM_RUNS):
        ...
        def preprocess_function(examples):
            num_choices = 2
            first_sentences = [[context] * num_choices for context in examples["premise"]]
            first_sentences = list(chain(*first_sentences))
            second_sentences = []
            for idx in range(len(examples["premise"])):
                second_sentences.append(examples["option1"][idx])
                second_sentences.append(examples["option2"][idx])
            # Tokenize
        ...
        print ("Eval acc printed:", metrics["eval_accuracy"])
        max_eval_samples = data_args.max_eval_samples if data_args.max_eval_samples is not None else len(eval_dataset)
        metrics["eval_samples"] = min(max_eval_samples, len(eval_dataset))

        trainer.log_metrics("eval", metrics)
        trainer.save_metrics("eval", metrics)
    # Print statistics over all accuracies
    print_avg_scores(accuracies)
